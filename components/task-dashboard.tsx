...
import { Task, TaskFilters, TaskFormInput, TaskView } from '@/lib/types';
...
export function TaskDashboard() {
  ...
  const starredCount = useMemo(() => tasks.filter((task) => task.is_starred).length, [tasks]);
        ...
        <aside className="hidden w-64 shrink-0 rounded-3xl bg-white p-5 shadow-card md:block">
          ...
          <nav className="mt-8 space-y-2">
            {sidebarItems.map((item) => (
              <button
                ...
                className={cn(
                  ...
                  view === item.key ? 'bg-brand-50 text-brand-600' : 'text-slate-600 hover:bg-slate-100',
                )}
              >
            ...
            <div className="mt-6 rounded-2xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">
              标签管理与设置在此 MVP 中整合到顶部筛选区与通知权限流程。
            </div>
            <button className="mt-2 flex items-center gap-2 rounded-2xl px-4 py-3 text-sm text-slate-500">
              <Settings size={16} /> 设置
            </button>
          </nav>
        </aside>
        ...
        <section className="flex-1 rounded-3xl bg-white p-5 shadow-card md:p-6">
            ...
            {sidebarItems.map((item) => (
              <button
                ...
                className={cn(
                  ...
                  view === item.key ? 'bg-brand-500 text-white' : 'bg-slate-100 text-slate-600',
                )}
              >
                {item.label}
              </button>
          ...
          <div className="mt-0 flex flex-col gap-4 border-b border-slate-100 pb-5 pt-5 md:mt-0 md:pt-0 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:flex">
              <select className="rounded-2xl border border-slate-200 px-4 py-3 text-sm" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value as TaskFilters['status'] })}>
                <option value="all">全部状态</option>
                <option value="pending">待处理</option>
                <option value="completed">已完成</option>
              </select>
              <select className="rounded-2xl border border-slate-200 px-4 py-3 text-sm" value={filters.priority} onChange={(e) => setFilters({ ...filters, priority: e.target.value as TaskFilters['priority'] })}>
              ...
              <select className="rounded-2xl border border-slate-200 px-4 py-3 text-sm" value={filters.due} onChange={(e) => setFilters({ ...filters, due: e.target.value as TaskFilters['due'] })}>
                <option value="all">全部时间</option>
                <option value="today">今日到期</option>
                <option value="overdue">已过期</option>
              </select>
              <select className="rounded-2xl border border-slate-200 px-4 py-3 text-sm" value={filters.tag} onChange={(e) => setFilters({ ...filters, tag: e.target.value })}>
              ...
              <button className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-medium" onClick={() => setFilters(defaultFilters)}>
                重置筛选
              </button>
              <button className="rounded-2xl bg-brand-500 px-4 py-3 text-sm font-medium text-white" onClick={() => { setActiveTask(null); setModalOpen(true); }}>
                新建任务
              </button>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[1.6fr_1fr]">
            <div className="rounded-3xl bg-slate-50 p-5">
              <p className="text-sm text-slate-500">当前视图</p>
              <h2 className="mt-1 text-2xl font-semibold text-slate-900">{viewLabelMap[view]}</h2>
              <p className="mt-2 text-sm text-slate-500">共 {visibleTasks.length} 条任务，支持新增、编辑、删除、完成、星标与提醒。</p>
            </div>
            <div className="rounded-3xl bg-slate-50 p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button className="rounded-2xl bg-slate-900 px-4 py-2 text-sm font-medium text-white" onClick={() => void requestNotifications()}>
                  开启浏览器通知
                </button>
                <button className="rounded-2xl bg-amber-100 px-4 py-2 text-sm font-medium text-amber-800" onClick={() => void createReminderDemo()}>
                  <Plus size={16} className="mr-1 inline-block" />创建 1 分钟测试提醒
                </button>
                <button className="rounded-2xl bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm" onClick={() => void refreshTasks()}>
                  <RefreshCcw size={16} className="mr-1 inline-block" />刷新
                </button>
              </div>
            </div>
          </div>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <span className={cn('rounded-full px-3 py-1 text-xs font-semibold', task.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700')}>
                        {task.status === 'completed' ? '已完成' : '待处理'}
                      </span>
                      <span className={cn('rounded-full px-3 py-1 text-xs font-semibold', task.priority === 'high' ? 'bg-rose-100 text-rose-700' : task.priority === 'medium' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-700')}>
                        {task.priority}
                      </span>
                    </div>
                    <p className="max-w-3xl text-sm leading-6 text-slate-600">{task.description || '暂无描述'}</p>
                    <div className="flex flex-wrap gap-2">
                      {task.tags.length > 0 ? task.tags.map((tag) => <span key={tag} className="rounded-full bg-white px-3 py-1 text-xs text-slate-500 shadow-sm">#{tag}</span>) : <span className="text-xs text-slate-400">暂无标签</span>}
                    </div>
                    <div className="grid gap-2 text-sm text-slate-500 sm:grid-cols-3">
                      ...
                      <p>星标状态：{task.is_starred ? '已星标' : '普通'}</p>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 lg:w-64 lg:justify-end">
                    <button className="rounded-full bg-white p-3 text-amber-500 shadow-sm" aria-label={task.is_starred ? '取消星标' : '加入星标'} onClick={() => void patchTask(`/api/tasks/${task.id}/${task.is_starred ? 'unstar' : 'star'}`, { method: 'POST' })}><Star size={16} fill={task.is_starred ? 'currentColor' : 'none'} /></button>
