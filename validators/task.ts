...
function validateReminderFields(
  ...
  if (value.reminderEnabled && !value.remindAt && !value.snoozeUntil) {
    ctx.addIssue({
      ...
      path: ['remindAt'],
      message: '开启提醒后必须提供提醒时间',
    });
  }
}

export const taskCreateSchema = taskBaseSchema.superRefine(validateReminderFields);

export const taskUpdateSchema = taskBaseSchema.partial().superRefine(validateReminderFields);
